...
3. 設定選「**完整套用**」（預設值），按「安裝」
   ...
   > **UAC 提權對話框會在這一步才出現**，不是雙擊執行時就跳出來。這是刻意
   > 的設計：安裝程式本身不整體要求系統管理員權限，只有真正需要權限的
   > 「把檔案放進 Program Files」這一步才會另外提權，其餘設定都在你原本
   > 登入的身分下完成，才不會寫進錯誤帳號的設定檔（見下方說明）。
...
v6.10.6 修好了這個症狀已確認的一種成因：舊版安裝程式雙擊就整支要求系統管理員
權限，如果一台電腦的日常帳號不是管理員、UAC 提權時輸入的是**另一個**管理員
帳號的密碼，安裝過程等於整段都在跑那個管理員帳號的身分，`Options.json` 會被
寫進那個帳號的 `%APPDATA%`，不是你實際在用 QTranslate 的人。v6.10.6 把安裝
程式改成只有真正需要權限的那一步（放檔案進 Program Files）才提權，其餘設定
都在原本身分正確的程序裡完成。**如果你是先前的版本遇到這個問題，先更新到
v6.10.6、選「完整套用」重裝一次，多半就會解決。**

如果**已經是 v6.10.6、也重裝過**，設定還是套用不上或每次重開就消失，那是
另一個尚未確認的成因——多半是 QTranslate 本身在那台電腦上事後存不回設定檔，
不是安裝時寫了什麼的問題，重裝解決不了。
