...
PDF 原本的斷行**——那只是顯示，不影響翻譯結果。中文 PDF 的段落也適用
（v6.10.8 起——更早的版本這個判斷只看字元數，中文字元的視覺寬度比英文
寬，純中文段落很難達到判斷門檻，折行邏輯基本上不會生效）。

> **已知限制**：畫面框選翻譯（OCR）如果框住的畫面同時有大標題跟小字
> 正文（例如網頁的標題+內文），因為兩者視覺行寬差異太大，目前的判斷
> 邏輯可能整段都不處理，換行原封不動保留。純文字段落（沒有大標題混
> 在一起）不受影響。這個限制還沒有修好。
...
`Double Ctrl`），「文字辨識」應該顯示 `Ctrl+Alt+S`（不是空白），這樣
就代表設定確實生效了。
